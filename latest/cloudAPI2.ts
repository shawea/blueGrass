    ...
    for (var methodName, i = 0; methodName = this.__methodList[i]; i++) {
        ...
        var wrapper = (function(instance, methodName) {
            ...
            return function() {
                ...
                else return call.instance.__callMethod(call.methodName, COARSRpc.toArray(arguments));
            };
        })(this, methodName);
...
COARSRpc.ServiceProxy.prototype.__callMethod = function(methodName, params, successHandler, exceptionHandler, completeHandler) {
    ...
    try {
        ...
        if (this.__isAsynchronous) {
            ...
            //Handle the response
            var instance = this;
            var requestInfo = {id:COARSRpc.requestCount}; //for XML-RPC since the 'request' object cannot contain request ID
...
COARSRpc.isJSON = function(string) { //from Prototype String.isJSON()
    var testStr = string.replace(/\\./g, '@').replace(/"[^"\\\n\r]*"/g, '');
    return (/^[,:{}\[\]0-9.\-+Eaeflnr-u \n\r\t]*$/).test(testStr);
};

COARSRpc.ServiceProxy.prototype.__evalJSON = function(json, sanitize) { //from Prototype String.evalJSON()
...
COARSRpc.dateToISO8601 = function(date) {
    ...
    return date.getUTCFullYear() + '-' +
            COARSRpc.zeroPad(date.getUTCMonth() + 1) + '-' +
            COARSRpc.zeroPad(date.getUTCDate()) + 'T' +
            COARSRpc.zeroPad(date.getUTCHours()) + ':' +
            COARSRpc.zeroPad(date.getUTCMinutes()) + ':' +
            COARSRpc.zeroPad(date.getUTCSeconds()) + '.' +
        //Prototype's Date.toJSON() method does not include milliseconds
            COARSRpc.zeroPad(date.getUTCMilliseconds(), 3);
};
